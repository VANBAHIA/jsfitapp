...
// ====================
// 1. ADIÇÕES AO SCRIPT.JS (Personal Trainer)
// ====================

// Adicionar ao objeto app no script.js:
...
// ====================
// 3. ADIÇÕES À RENDERIZAÇÃO DA LISTA DE PLANOS
// ====================

// Atualizar a função renderPlanList para incluir botão de compartilhamento:
renderPlanList() {
    const container = document.getElementById('planListContent');
    if (!container) return;
    ...
    const sharedPlans = this.getSharedPlansFromStorage();
    const sharedPlanIds = Object.keys(sharedPlans);
    ...
    if (sharedPlanIds.length > 0) {
        container.innerHTML += `
            <div class="shared-plans-section">
                ...
                ${this.getSharedPlansList().map(shared => `
...
async init() {
    ...
    try {
        ...
        console.log('Status da API de compartilhamento:', apiStatus ? 'Online' : 'Offline');
    } catch (error) {
